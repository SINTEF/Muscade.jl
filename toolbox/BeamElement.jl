...
-   `ε :: NTuple{nXder, 𝕣}` contains the axial strain of the element and its time-derivatives. `nXder` depends on the solver. For a [`SweepX{2}`](@ref) analysis, `nXder=3` and ε[1] contains the strain, ε[2] the strain-rate and ε[3] the second time-derivative of the strain.
-   `κ :: NTuple{nXder,SVector{3,𝕣}}`. The first element of κ contains the twist/curvatures at the centroid of the element. Twist is defined about the tangential direction of the element, and curvatures are defined as rate of rotation along the element. Note that κ[3]>0 implies +2 direction is inside curve, and κ[2]>0 implies -3 direction is inside curve. The second SVector{3,𝕣} in κ contains the time derivatives of twist/curvatures, the third SVector{3,𝕣} the second time-derivatives, etc. 
-   `rₛₘ :: NTuple{nXder,SMatrix{3,3,𝕣,9}}`. The first element of the Tuple is the "from local-to-global" rotation matrix describing the orientation of the element local coordinate system. The remaining elements of the Tuple are its first and second time-derivatives.
-   `@request gp(x)` to get `gp.x:: NTuple{nXder,Tuple{SVector{3,𝕣}}` contains the motions of Gauss points. The first element of the Tuple contains the three absolute positions of the Gauss point, the second the three velocities, the third one the three accelerations, etc. 
-   `@request gp(κgp)` to get `gp.κgp` that contains twists and curvatures at each Gauss point (see κ above). Same type and structure as for `gp(x)`. 
-   `@request gp(fₑ)` to get `gp[:resultants][:fₑ]) :: SVector{3, 𝕣}` contains the three components of the external forces at the Gauss point, calculated in the `resultants` function of the beam element. 
-   `@request gp(mₑ)` to get `gp[:resultants][:mₑ]) :: SVector{3, 𝕣}` contains external torsional and bending moments about the three local directions of the element. 
-   `@request gp(fᵢ)` to get `gp[:resultants][:fᵢ]) :: 𝕣` contains the internal axial force at the Gauss points
-   `@request gp(mᵢ)` to get `gp[:resultants][:mᵢ]) :: SVector{3, 𝕣}` contains internal torsional and bending moments about the three local directions of the element. 
