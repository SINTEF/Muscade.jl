...
# MUST be used by elements to unpack variables.  Today, the various derivatives are packed into tuples.  Would we use Adiff tomorrow, allowing
...
lagrangian(      ::eltyp,args...) where{eltyp<:AbstractElement}    = muscadeerror(@sprintf "no method lagrangian for %s, or wrong list of further arguments: %s" eltyp (typeof.(args)))
initstate(       ::eltyp)         where{eltyp<:AbstractElement}    = nostate
