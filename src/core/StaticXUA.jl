...
function StaticXUA(pstate,dbg;model::Model,time::AbstractVector{𝕣},
    ...
    maxiter::ℤ=50,maxΔy::ℝ=1e-5,maxresidual::ℝ=∞,verbose::𝕓=true)
    verbose && @printf "    StaticX solver\n\n"
    dis                = initial.dis
    ...
    Δy_a               = Matrix{𝕣1}(undef,length(time))
    Δy²,Ly²            = Vector{𝕣}(undef,length(time)),Vector{𝕣}(undef,length(time))
    for iiter          = 1:maxiter
        verbose && @printf "    iteration %3d" iiter
        La            .= 0
        Laa           .= 0
        for step     ∈ eachindex(time)
            ...
            Δy_a[step] = try asm.Lyy\asm.Lya catch; muscadeerror(@sprintf("Incremental solution failed at step=%i, iiter=%i",step,iiter)) end
            La       .+= asm.La + asm.Lay ∘₁ Δy[  step]
            Laa      .+= asm.La + asm.Lay ∘₁ Δy_a[step]
            Δy²[step],Ly²[step] = sum(Δy.^2),sum(asm.Ly.^2)
        end    
        Δa           .+= Laa\La 
        Δa²,La²        = sum(Δa.^2),sum(La.^2)
        for step       ∈ eachindex(time)
            ΔY         = Δy[step] - Δy_a[step] ∘₁ Δa
            decrement!(state[step],ΔY,Ydofgr)
            decrement!(state[step],Δa,Adofgr)
        end    
        if all(Δy².≤cΔy²) && all(Ly².≤cLy²) && Δa².≤cΔa² && La².≤cLa² 
            verbose && @printf " converged in %3d iterations. |Δy|=%7.1e |Ly|=%7.1e |Δa|=%7.1e |La|=%7.1e\n" iiter √(maximum(Δy²)) √(maximum(Ly²)) √(Δa²) √(La²)
