...
function draw_(axe,dis::EletypDisassembler,eleobj,iele,state,χ,dbg;kwargs...) 
    # typestable kernel
    for ie ∈ iele
        index = dis.index[ie]
        ...
        Ae    = state.A[index.A]
        eo    = eleobj[ie]
        draw(axe, eo, Λe,Xe,Ue,Ae, state.time,χ[iele],state.SP,(dbg...,iele=ie);kwargs...)
    end
    return
end
function draw(axe,state::State,ieletyp::𝕫; iele::ℤ1=1:length(state.model.ele[ieletyp]),kwargs...)
    ...
    draw_(axe,dis,eleobj,iele,state,state.χ[ieletyp],(ieletyp=ieletyp,);kwargs...) # call kernel
    return
end
