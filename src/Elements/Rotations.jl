...
# 1/coef: 0!*5, -2!*7, 4!*9,-6!*11, 8!*13, -10!*15 ...  Approximation is pretty rough at π... (1e-4 error)
function sinc1⁗(x) 
    x² = x*x
...
function scac(x)
    dx = x-1  # deliberately a long Taylor series: this function will be adiffed at least to 2nd order, up to 4th order
    if abs(dx)>1e-2 
        sinc1(acos(x))
    else
#        1 + dx/3 - 2/90*dx^2 + 0.0052911879917544626*dx^3 -0.0016229317117234072*dx^4 + 0.0005625*dx^5 
...
Rodrigues⁻¹(m) = spin⁻¹(m)/scac((trace(m)-1)/2)   # NB: 0/0 for 180d turn
