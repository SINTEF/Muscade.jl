...
However, for advanced element implementation, `residual` or `lagrangian` may themselves use automatic differntiation, and the following provides the necesary guidance-
!!! Warning
    Read and understand the section on "perturbation collision" before using any of the techniques described below.
...
- `P` is the *precedence*, or order of differentiation.
...
This structure is nested to represent higher derivatives, so `R` can itself be a `∂ℝ`. The rule is that in any `∂ℝ{P,N,R}`, we must have `P==1+predence(R)`, where the precedence of `𝕣` (a alias for `Float64`) is 0. A variable `v` of precedence 2 is printed as `∂₂⟨∂₁⟨v.x.x|v.x.dx⟩|∂₁⟨x.dxᵢ.x|x.dxᵢ.dx⟩ ∀i⟩`. For example, 
...
In the following, the informal notations `∂₂⟨∂₁⟨v.x.x|v.x.dx⟩|∂₁⟨v.dx.x|v.dx.dx⟩⟩` or `∂₂⟨∂₁⟨v|vₓ⟩|∂₁⟨vₓ|vₓₓ⟩⟩` may be used.  Both notations gloss over the fact that `dx` is a vector.
...
[`Muscade.variate`](@ref) here builds a `SVector` of `∂ℝ{P,N,R}`, where `R == eltype(x)`, `P == Muscade.precedence(x)+1` (the precedence of an `𝕣` is 0) and `N` is the length of `x`.
. `Muscade` developers speak of `x1` being the "adiffed" form of `x`". The partials are "seeded", so that `∂{P,N}(x1)[i]` is a `SVector{N}` of `zero(R)`, except for the `i`-th term that is `one(R)`.  
...
[`value`](@ref)`{P}(y1)`, where `y1` is a `StaticArray{S,∂{P,N,R}}`, retrieves the value `y`, of type `StaticArray{S,R}`. [`∂`](@ref)`{P,N}(y1)`, where `y1` is a `StaticArray{S,∂{P,N,R}}`, retrieves the gradient `yₓ`, of type `StaticArray{Syₓ,R}`: `yₓ` has one more dimension than `y`, of size `N`, corresponding to the partial derivatives with respect to the `SVector` `x`.
...
```julia
using StaticArrays
using Muscade: variate,variate_indices,value,∂,δ,𝕣

res(x,u)    = x.*x .+ sum(u) # SVector-valued 
x           = SVector(1.,2.)
u           = SVector(3.,4.,5.)
P,N,(x1,u1) = variate((x,u))
...
Higher order derivatives are obtained by using `variate{O}(...)` where `O` (defaulting to 1, as seen in previous examples) is the order of differentiation. Note that the `∂ℝ` object stores `(N+1)^O` numbers, so computations can quickly become prohibitively slow.
...
The computations then result in `ℓ2=∂₂⟨∂₁⟨ℓ|ℓₓ⟩|∂₁⟨ℓₓ|ℓₓₓ⟩⟩`, of precedence 2 "inherited" from `x2`. To unpack this nested structure, we first use `value{2}(ℓ2) → ∂₁⟨ℓ|ℓₓ⟩` and `∂{2,N}(ℓ2) → ∂₁⟨ℓₓ|ℓₓₓ⟩`.  On these, we then use `value{1}(∂₁⟨ℓ|ℓₓ⟩) → ℓ`, `∂{1,N}(∂₁⟨ℓ|ℓₓ⟩) → ℓₓ`, `value{1}(∂₁⟨ℓₓ|ℓₓₓ⟩) → ℓₓ` and `∂{1,N}(∂₁⟨ℓₓ|ℓₓₓ⟩) → ℓₓₓ`. `ℓₓ` is stored twice in `ℓ2` and so can be extracted by two different paths.  

A caution about performance: higher derivatives means that variated numbers carry a significant amount of information, and all operations on them become slower.  Also, because the compiler rolls out all the loops operations with variated numbers, compilation can become very heavy. 

## Nested derivatives

Neste derivatives occur when a function, which uses automatic differentiation internaly is itself called with automatic differentiation. An example in `Muscade.Toolbox` is the implementation of `EulerBeam3D`'s method for `Muscade.residual`. This beam element uses a co-rotated reference system, which pauses the problem of how to transform, for example, bending moments `mᵢ` at Gauss quadrature points, back into nodal loads `R`.  Mathematicaly, their contribution to the residual is `mᵢ ∘₁ κ∂X₀`, where `κ∂X₀` is the Jacobian of the curvatures `κ` with respect to the 0-th order time derivative `X₀` of the element's `X`-dofs.

Here is a simpler (if contrived) example:
...
The function `f` calls `variate`, on `x1` which has itself been obtained by calling `variate`.  `x`, `x1` and `x2` have precedence 0, 1 and 2, respectively. The unpacking of the variables is as for higher order derivatives, but here, each segment of code takes care to unpack what it has variated.
...
Nested differentiation in all its forms carries the danger of "perturbation confusion". `residual` and `lagrangian` are called by various `Muscade`-solvers with input variables `Λ`, `X`, `U`, `A` and `t` that may or may not be variated, and that to different orders.

Here is an example of how things go wrong (variable names include the precedence. This is only done here to help readability. When implementing an element in `Muscade` the precedence of input variables is not known to the developer).
...
function residual(x1,u) 
    P,N,u1 = variate(u) # wrong implmentation
...
function residual(x1,u) 
    P,N,u2 = variate(u,context=x1) 
    g2     = sum(x1)*sin(sum(u2)) 
    g1ᵤ    = ∂{P,N}(g2) 
...
Thanks to the `context` input, `variate` can now detect that although `u` has precedence 0, `x1` already "occupies" precedence 1.  Variate thus returns `u2`, of precedence `P==2`. `u2` has the structure `∂₂⟨∂₁⟨u|uₓ⟩|∂₁⟨uₓ|uₓₓ⟩⟩`, where `uₓ` and `uᵤₓ` are zero (`u` does not depend on `x`).  This operates correctly with `∂₁⟨x|xₓ⟩`, in spite of the different precedences of `x1` and `u2`.

When implementing an element, the safest is to set `(Λ,X,U,A,t)` (`lagrangian`) or `(X,U,A,t)` (`residual`) as `context`. But if, for example, a solver was calling `residual` with only `A` variated, and a `∂ᵢ` issued by a call to `variate` within `residual` never touches `A` or any variable influenced by `A`, then omitting `A` from the `context` allows `variate` to issue a variable of lower precedence, enabling faster execution.

!!! important
    Never allow two `∂ᵢ` issuing from different calls to [`variate`](@ref), [`variate0`](@ref), [`revariate`](@ref) and [`motion`](@ref) to mix.

Consider using `let` blocks to create a local scope containing "dangerous variables".

```julia
function f(x) 
    gₓ = let P,N,xP = variate(x)
...
If `x` (lower case) is the i-th degree of freedom, then `X_[i]` has the structure  `∂₂⟨∂₁⟨x₀|x₁⟩|∂₁⟨x₁|x₂⟩⟩`, so the number of partials is 1 for both `∂₂` and `∂₁`: while higher order derivatives in general are expensive to compute in general `motion` is affordable. `ε_` has the same structure.

If `X₀`,`X₁`,`X₂` or `context` contains variables of precedence for example 1, then `motion` returns `∂₃⟨∂₂⟨x₀|x₁⟩|∂₂⟨x₁|x₂⟩⟩`.

## Chain rule

Applying the chain rule *can*, under the right circumstances accelerate computations. We are differentiating a function `f` with respect to a long vector `x`. Withinf `f`, a computationaly expensive subfunction `z=g(y)` is only function of a significantly shorter vector `y`.
...
``\frac{\partial z_i}{\partial x_k} = \sum_j \frac{\partial z_i}{\partial y_j}\cdot\frac{\partial y_j}{\partial x_i}``. When diffrentiating to an arbitrary order, the computation `z=g(y)` yields a variated number `z` that contains value and multiple derivatives. This data can be used to evaluate a Taylor expansion of `g`, with respect to `y-VALUE(y)`.
...
Under the hood, the code of `apply` is
