...
struct OneDofOscillator <: AbstractElement
    K   :: SMatrix{1,1,𝕣}
    C   :: SMatrix{1,1,𝕣}
    M   :: SMatrix{1,1,𝕣}
end
OneDofOscillator(nod::Vector{Node};K,C,M  )  = OneDofOscillator(K,C,M)

Muscade.doflist(::Type{<:OneDofOscillator}) = (  inod = (1,1,1,1),                                             
                                                 class= (:X,:U,:A,:A), 
                                                 field= (:surge,:surge,:M,:C))

@espy function Muscade.residual(o::OneDofOscillator,   X,U,A,t,SP,dbg) 
    x,x′,x″    = ∂0(X),∂1(X),∂2(X)   
    ☼u         = ∂0(U)
    a          = exp10.(A)
    ☼r₂        = (o.M.*a[@SVector [i for i∈1:1 ]])∘x″
    ☼r₁        = (o.C.*a[@SVector [i for i∈2:2]])∘x′
    ☼r₀        = o.K∘x
    return r₀+r₁+r₂-u,  noFB
end
...
model     = Model(:OneDofOscillator)
n1        = addnode!(model,𝕣[0,0,0])  
e1        = addelement!(model,OneDofOscillator,[n1]; K,C,M)
...
maxDevToModel = 0.0; 
devToModelM = 1. .+ maxDevToModel*((rand(1).-.5)*2); 
devToModelC = 1. .+ maxDevToModel*((rand(1).-.5)*2);
Mguess = M .* devToModelM[@SVector [i for i∈1:1 ]]
Cguess = C .* devToModelC[@SVector [i for i∈1:1 ]]

# Create XUA model
modelXUA     = Model(:OneDofOscillator)
n1        = addnode!(modelXUA,𝕣[0,0,0])  
e1        = addelement!(modelXUA,OneDofOscillator,[n1]; K,C=Cguess,M=Mguess)
...
QMaa      = @SVector [.1 ^-2 for i=1:1]  
e4        = addelement!(modelXUA,SingleDecayAcost  ,[n1];          field=:M,fac,           cost=(a  )-> 0.5*QMaa[1]/length(T)*a^2) 
...
Mest      = Mguess .* exp10.(SVector{1}(stateXUA[niter][1].A[1])) 
Cest      = Cguess .* exp10.(SVector{1}(stateXUA[niter][1].A[2])) 
@show Mguess ./ M
@show Mest ./ M
@show Cguess ./ C
@show Cest ./ C;
