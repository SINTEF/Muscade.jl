...
# Shape functions for a beam element with support ζ∈[-1/2,1/2]                          
...
# First derivatives ∂N/∂ζ used to compute elongation and torsion
# (For an element of length L, use ∂N/∂x=∂N/∂ζ/L)
Bₐ₁(ζ) = -1        
Bₐ₂(ζ) =  1

# Second derivatives ∂²N/∂ζ² used to compute curvature
# (For an element of length L, use ∂²N/∂x²=∂²N/∂ζ²/L²)
Bᵤ₁(ζ) =   12ζ
Bᵥ₁(ζ) =    6ζ-1
Bᵤ₂(ζ) =  -12ζ  
...
function EulerBeam3D(nod::Vector{Node};mat,orient2::SVector{ndim,𝕣}=SVector(0.,1.,0.))
    # Constructor for the EulerBeam3D element. 
    # Arguments: node coordinates and direction of the first bending axis in the global coordinate system.  
    c       = coord(nod)
...
@espy function Muscade.residual(o::EulerBeam3D,   X,U,A,t,SP,dbg) 
    # Fetch the element properties (see definition in EulerBeam3D constructor code)
    ...
    ζgp,ζnod,dL      = o.ζgp,o.ζnod,o.dL        # Gauss points properties and shape functions
    ...
    R  = sum(gpᵢ.Rₗ for gpᵢ∈gp) ∘ T # Nodal loads from Gauss quadrature and tuples of Rl inside gp. 
    return R,noFB  
end
