...
(g,h)=promote(dx1,x)
R1   =        ∂ℝ{1,2,Float64}
R2   = ∂ℝ{2,3,∂ℝ{1,2,Float64}}
Rx   = ∂ℝ{2,3,∂ℝ{1,3,Float64}}
...
Δ   = Muscade.δ_{1,2,𝕣}()
C1  = Muscade.variate_{Muscade.precedence(Δ),2}(SVector(3.,4.))
C   = Muscade.variate_{Muscade.precedence(C1)+1,2}(C1)
...
ob = Muscade.variate_{Muscade.precedence(oa   )+1}(oa)
oc = Muscade.variate_{Muscade.precedence(oa,ob)+1}(ob)
od = oa+oc
oe = od+ob
oj = od^2
og = 2^od
...
@testset "Adiff" begin
    @testset "Adiff construct and promote" begin
        ...
        @test h          === x
        @test promote_type(R2,R1) == R2
        @test_throws Exception promote_type(R2,Rx) 
    end

    @testset "Adiff extraction" begin
        @test Δ            === SVector(∂𝕣12(0.0, SVector(1.0, 0.0)),∂𝕣12(0.0, SVector(0.0, 1.0)))
        @test Muscade.precedence(Δ) == 1
        @test typeof(C)    == SVector{2, ∂ℝ{2, 2, ∂ℝ{1, 2, Float64}}}#Array{∂ℝ{3,2,∂𝕣22},1}
        @test C[1]         === ∂ℝ{2, 2, ∂ℝ{1, 2, Float64}}(∂ℝ{1, 2, Float64}(3.0, [1.0, 0.0]), ∂ℝ{1, 2, Float64}[∂ℝ{1, 2, Float64}(1.0, [0.0, 0.0]), ∂ℝ{1, 2, Float64}(0.0, [0.0, 0.0])])
        @test vC[1]        === ∂ℝ{1, 2, Float64}(3.0, [1.0, 0.0])
        @test ∂C[1]        === ∂ℝ{1, 2, Float64}(1.0, [0.0, 0.0])
...
#        @test ∂∂C          === zeros(2,2,2)   broken=true # extract partial derivatives of an SMatrix or higher order
    ...
    @testset "Adiff operations" begin
        ...
        @test ob === ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [1.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(1.0, [0.0])])
        @test oc === ∂ℝ{3, 1, ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}}(∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [1.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(1.0, [0.0])]), ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}[∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [0.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(0.0, [0.0])])])
        @test od === ∂ℝ{3, 1, ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}}(∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(2.0, [2.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(1.0, [0.0])]), ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}[∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [0.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(0.0, [0.0])])])
        @test oe === ∂ℝ{3, 1, ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}}(∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(3.0, [3.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(2.0, [0.0])]), ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}[∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [0.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(0.0, [0.0])])])        
        @test og === ∂ℝ{3, 1, ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}}(∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(4.0, [5.545177444479562]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(2.772588722239781, [3.843624111345611])]), ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}[∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(2.772588722239781, [3.843624111345611]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(1.9218120556728056, [2.6641972159114355])])])        
        @test oj === od*od
        @test ok === ∂ℝ{3, 1, ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}}(∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(1.0, [1.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(0.0, [0.0])]), ∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}[∂ℝ{2, 1, ∂ℝ{1, 1, Float64}}(∂ℝ{1, 1, Float64}(0.0, [0.0]), ∂ℝ{1, 1, Float64}[∂ℝ{1, 1, Float64}(0.0, [0.0])])])
