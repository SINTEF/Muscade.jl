...
el = ElementCost(model.nod;requested=@request(Fh),cost,ElementType=AnchorLine, 
                 Δxₘtop=[5.,0,0], xₘbot=[250.,0], L=290., buoyancy=-5e3)
d  = doflist(typeof(el))
e  = espyable(typeof(el))
Nx,Nu,Na        = 3,0,2   
Nz              = 2Nx+Nu+Na     
...
L  = lagrangian(el, Λ+ΔΛ, (∂0(X)+ΔX,),(∂0(U)+ΔU,),A+ΔA, 0.,0.,(testall=true,))                 

@testset "ElementCost" begin
     @test d == (inod = (1, 1, 1, 2, 2), class = (:X, :X, :X, :A, :A), field = (:tx1, :tx2, :rx3, :ΔL, :Δbuoyancy))
     @test e == (cost = (), lagrangian = (Xtop = (3,), ΔXtop = (3,), ΔXchain = (2,), xaf = (), cr = (), Fh = (), ltf = ()))
