...
@testset "model.nod" begin
    @test model.nod[1].nodID == NodID(1)
...
@testset "model.ele" begin
    @test model.ele[1][1].eleID    == EleID(1,1)
    ...
    @test model.ele[2][1].eleID       == EleID(2,1)
...
@testset "model.dof" begin
    @test model.dof[DofID(:X,1)].dofID    == DofID(:X,1)
